...
//! Copyright (c) 2024, Optimatist Technology
//! All rights reserved.
//!
use std::{
...
pub struct PollBuffer {
    ...
    pub result_container: Arc<Mutex<Option<Vec<u8>>>>,
}
...
pub struct BpfCtx<'a, 'store, State> {
    ...
    pub store: Option<&'store mut Store<State>>,
    pub instance: Option<&'a Instance>,
}

impl<State> BpfCtx<'_, '_, State> {
    /// Create an AppState
    pub fn new(
        //wasi: WasiCtx,
...
//pub(crate) type CallerType<'a, 'store> = Caller<'a, BpfCtx<'store>>;
